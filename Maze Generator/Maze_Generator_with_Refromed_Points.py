...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 10 14:05:19 2022
...
def build_grid(x, y, w):
    for i in range(1,21):
        ...
        for j in range(1, 21):
            ...
            pygame.draw.line(screen, BLACK, [x + w, y + w], [x, y + w])   # bottom of cell
...
def single_cell( x, y):
    pygame.draw.rect(screen, WHITE, (x +1, y +1, 18, 18), 0)          # draw a single width cell
    pygame.display.update()


def backtracking_cell(x, y):
    pygame.draw.rect(screen, WHITE, (x +1, y +1, 18, 18), 0)        # used to re-colour the path after single_cell
    pygame.display.update()                                        # has visited cell


def solution_cell(x,y):
    pygame.draw.rect(screen, BLACK, (x+8, y+8, 5, 5), 0)             # used to show the solution
    pygame.display.update()                                        # has visited cell
...
def carve_out_maze(x,y):
    ...
    visited.append((x,y))    
    n=0                                                             # add starting cell to visited list
    while len(stack) > 0:                                           # loop until stack is empty
        #time.sleep(.05)                                              # slow program now a bit
        ...
        if len(cell) > 0:                                          # check to see if cell list is empty
            ...
            n=n+1                   # select one of the cell randomly
